...
- PRD pointer/metadata (json): scripts/ralph/prd.json
- Resume from the first non-complete story; do not redo completed stories.
- Append learnings to scripts/ralph/progress.txt.

Verification requirements (MANDATORY):
- Start dev server using:
  npm run dev -- --hostname 127.0.0.1 --port 3000
- After any UI or routing change:
  run `npm run test:browser`
- Do NOT mark passes=true unless browser verification passes.
- If tests fail, diagnose and fix before continuing.
- Append each iteration’s learnings to scripts/ralph/progress.txt.
